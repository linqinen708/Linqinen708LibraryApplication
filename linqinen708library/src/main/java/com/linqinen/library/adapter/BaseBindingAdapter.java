    ...
    public long getItemId(int position) {
        ...
        if (isShowFooterView) {
            if (position == items.size()) {
                return R.layout.adapter_footer_view;
            } else {
                position--;
            }
        }
...
//            LogT.i("加载footview");
            return new FooterViewHolder(LayoutInflater.from(mContext).inflate(
                    getFooterViewLayoutResId() == 0 ? R.layout.adapter_footer_view : getFooterViewLayoutResId(),
                    parent, false));
...
//                this.onBindItem(binding, this.items.get(position));
                ...
                TextView tvFootView = holder.itemView.findViewById(R.id.tv_footer_view);
                ImageView ivEmpty = holder.itemView.findViewById(R.id.iv_empty);
                LogT.i("数据大小:" + items.size());
                if(items.isEmpty()){
