...
/**
 * Created by 林 on 2017/5/31.
 * 工具类，实现公农历互转
 *
 * 使用方式：
 * String lunarCalendar = "农历" + LunarCalendar.solarToLunar(year, currentMonth, currentDate);
 * textView.setText(lunarCalendar);
 */
...
//        LogT.i("闰月leapMonth:" + leapMonth);
        ...
        /*当前月超过闰月，要校正
        （源代码是leapMonth != 0 && iMonth > leapMonth，发现有问题，因为闰月的月份会出现两次，而用原来的代码会出现第一次的闰月少了一个月
        ，改为leapMonth != 0 && iMonth > leapMonth +1）*/
//        LogT.i("当前月iMonth:" + iMonth);
